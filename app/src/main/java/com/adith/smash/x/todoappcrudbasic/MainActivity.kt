package com.adith.smash.x.todoappcrudbasic
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_update.setOnClickListener {
            ...
            if (kalimatAwal.isBlank() || kalimatUpdate.isBlank()){
                toastData("Kolum tidak boleh kosong")
            } else {
    ...
    private fun hapusData(nama: String) {
        ...
        val dataListener = object : ValueEventListener{
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.childrenCount > 0){
                    // jika data tersebut tidak ada, maka hapus field nama yang ada di dalam tabel "Daftar Nama"
                    databaseRef.child("Daftar Nama").child(nama)
                        .removeValue()
                        .addOnCompleteListener { task ->
                            if (task.isSuccessful)toastData("$nama telah di hapus")
                        }
                } else {
        ...
        val cekData = databaseRef.child("Daftar Nama")
            .child(nama)
        // addValueEventListener itu menjalankan Listener terus menerus selama data yang di inputkan sama
    ...
    private fun cekDataKalimat() {
        readDataListener = object : ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                // check apakah data ada di dalam database
                if (snapshot.childrenCount > 0){
                    var textData = ""
        ...
        // cek data menuju database firebase di tabel "Daftar Nama"
    ...
    private fun tambahData(nama: String) {
                ...
                if (snapshot.childrenCount < 1){
                    ...
                    tambahData.addOnCompleteListener { task ->
                        if (task.isSuccessful){
                            toastData("$nama telah di tambahkan dalam database")
                        } else {
        ...
        // untuk mengecheck tabel daftar nama apakah data yang ingin diinputkan ke tabel tersebut sudah ada
        databaseRef.child("Daftar Nama")
